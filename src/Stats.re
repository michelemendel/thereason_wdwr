...
let lineReducer = (acc: list(order), line: string): list(order) => {
  ...
  if (Belt.Array.length(items) != 7) {
    ...
    let orderRecord =
      map2(initial, optInt(items[0]), (result, quantity) => {...result, quantity})
      ->map2(Size.fromString(items[1]), (result, size) => {...result, size})
      ->map2(Color.fromString(items[2]), (result, color) => {...result, color})
      ->map2(Pattern.fromString(items[3]), (result, pattern) => {...result, pattern})
      ->map2(Collar.fromString(items[4]), (result, collar) => {...result, collar})
      ->map2(Sleeve.fromString(items[5]), (result, sleeve) => {...result, sleeve})
      ->map2(Cuff.fromString(items[6]), (result, cuff) => {...result, cuff});
...
let printStatistics = (orders: list(order)): unit => {
  let colorDistribution =
    Belt.List.reduce(orders, Belt.Map.make(~id=(module ColorComparator)), colorReducer);
  Js.log2("Color", "Quantity");
  Belt.Map.forEach(colorDistribution, (key, value) => Js.log2(Color.toString(key), value));
};
...
| (Some(node), Some(prog), _) => Js.log("Usage: " ++ node ++ " " ++ prog ++ " inputfile.csv")
| (_, _, _) => Js.log("How did you get here without NodeJS or a program to run?")
};